...
def api_dashboard_stats(request):
    ...
    # Variations
    etudiants_semaine_derniere = total_etudiants
    ...
    for etudiant in Etudiant.objects.all()[:5]:
        ...
        absences_non_just = Absence.objects.filter(etudiant=etudiant, justifiee=False).count()
    ...
    top_modules = Absence.objects.values(
        ...
        non_justifiees=Count('id', filter=Q(justifiee=False))
    ).order_by('-total')[:5]
